...
public class Work implements Runnable {
	...
	//variabile che contiene il podio da inviare al server
        ArrayList<UserValoreClassifica> ClassificaNotifiche = new ArrayList<>();

        try {
            ReadLockClassifica.lock();
            for(int i = 0; i<Dim; i++) {ClassificaNotifiche.add(Classifica.get(i));}//inserisco le prime posizioni
        }
        finally {ReadLockClassifica.unlock();}

        for(Utente u : Registrati.values()) {//per ogni utente registarto

            try {
                u.getReadLock().lock();
                HashMap<UUID, InfoLogin> MapStub = u.getLoginChannel();//recupero le info di login dell utente

                for(InfoLogin i : MapStub.values()) {//recupero lo stub associato ad ogni connessione 

                    NotificaClient stub = i.getStub();
                    if(stub != null) {//se l'utente è in gioco quindi stub != null
                        try {stub.SendNotifica(ClassificaNotifiche);}//invio la notifica
                        catch (Exception e){e.printStackTrace();}
                    }
                }
            }
